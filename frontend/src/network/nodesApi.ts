...
export async function getLoggedInUser(): Promise<User> {
  const response = await fetchData(
    `http://127.0.0.1:5000/api/routes/users/authorisedUser`,
    {
      method: "GET",
      credentials: "include",
      headers: {
...
export async function signupFunctionFrontend(
  ...
  const response = await fetchData(
    `http://127.0.0.1:5000/api/routes/users/signup`,
...
export async function loginUserInFrontend(
  ...
  const response = await fetchData(
    `http://127.0.0.1:5000/api/routes/users/login`,
...
export async function logoutUserInFrontend() {
  const response = await fetchData(
    `http://127.0.0.1:5000/api/routes/users/logout`,
...
export async function fetchNodes(pageNumber:number): Promise<MainPageReturnType> {
  const response = await fetchData(`http://127.0.0.1:5000/api/routes/${pageNumber}`, {
    method: "GET",
    credentials: "include",
    headers: {
      "Content-Type": "application/json",
...
export async function createNote(note: RoleInput): Promise<NodeType> {
  const response = await fetchData("http://127.0.0.1:5000/api/routes", {
    method: "POST",
    credentials: "include",
    headers: {
      "Content-Type": "application/json",
...
export async function deleteNode(noteId: string): Promise<void> {
  await fetchData("http://127.0.0.1:5000/api/routes/" + noteId, {
...
export async function updateNode(
  ...
  const response = await fetchData(
    "http://127.0.0.1:5000/api/routes/" + noteId,
    {
      method: "PATCH",
      credentials: "include",
      headers: {
